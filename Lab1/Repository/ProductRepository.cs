...
namespace Lab1.Repository
{
    public class ProductRepository: IProductRepository
    {
        ...
        public Product GetById(int id) { 
            Product? product = context.Products.Include(p => p.Category).FirstOrDefault(p => p.Id == id); 
            return product;
        }
        public List<Product> GetByName(string name)
        {
            List<Product> products = context.Products.Include(p => p.Category).Where(products => products.Name == name).ToList();
            return products;
        }
